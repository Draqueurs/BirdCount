...
class TwoScrollableFrame(tkinter.Frame, appearance_mode.CTkAppearanceModeBaseClass, scaling.CTkScalingBaseClass):
    def __init__(self,
                 master: any,
                 width: int = 200,
                 height: int = 200,
                 ...
                 label_text: str = "",
                 label_font: Optional[Union[tuple, font.CTkFont]] = None,
        ...
        self._parent_frame = ctk_frame.CTkFrame(master=master, width=0, height=0, corner_radius=corner_radius,
        ...
        self._scrollbar_hor = ctk_scrollbar.CTkScrollbar(master=self._parent_frame, orientation="horizontal", command=self._parent_canvas.xview,
                                        fg_color=scrollbar_fg_color, button_color=scrollbar_button_color, button_hover_color=scrollbar_button_hover_color)
        self._parent_canvas.configure(xscrollcommand=self._scrollbar_hor.set)
        self._scrollbar_ver = ctk_scrollbar.CTkScrollbar(master=self._parent_frame, orientation="vertical", command=self._parent_canvas.yview,
                                        fg_color=scrollbar_fg_color, button_color=scrollbar_button_color, button_hover_color=scrollbar_button_hover_color)
        self._parent_canvas.configure(yscrollcommand=self._scrollbar_ver.set)

        self._label_text = label_text
        self._label = ctk_label.CTkLabel(self._parent_frame, text=label_text, anchor=label_anchor, font=label_font,
                               corner_radius=self._parent_frame.cget("corner_radius"), text_color=label_text_color,
                               fg_color=theme.ThemeManager.theme["CTkScrollableFrame"]["label_fg_color"] if label_fg_color is None else label_fg_color)

        tkinter.Frame.__init__(self, master=self._parent_canvas, highlightthickness=0)
        appearance_mode.CTkAppearanceModeBaseClass.__init__(self)
        scaling.CTkScalingBaseClass.__init__(self, scaling_type="widget")
        ...
        self._create_window_id_hor = self._parent_canvas.create_window(0, 0, window=self, anchor="nw")
        self._create_window_id_ver = self._parent_canvas.create_window(0, 0, window=self, anchor="nw")
    ...
    def destroy(self):
        ...
        appearance_mode.CTkAppearanceModeBaseClass.destroy(self)
        scaling.CTkScalingBaseClass.destroy(self)
    ...
    def _create_grid(self):
        ...
        self._scrollbar_hor.grid(row=2, column=0, sticky="nsew", padx=border_spacing)
            
        self._scrollbar_ver.grid(row=1, column=1, sticky="nsew", pady=border_spacing)
    ...
    def configure(self, **kwargs):
        ...
        if "fg_color" in kwargs:
            ...
            for child in self.winfo_children():
                if isinstance(child, core_widget_classes.CTkBaseClass):
                    child.configure(bg_color=self._parent_frame.cget("fg_color"))

        if "scrollbar_fg_color" in kwargs:
            self._scrollbar_hor.configure(fg_color=kwargs.pop("scrollbar_fg_color"))
            self._scrollbar_ver.configure(fg_color=kwargs.pop("scrollbar_fg_color"))

        if "scrollbar_button_color" in kwargs:
            self._scrollbar_hor.configure(button_color=kwargs.pop("scrollbar_button_color"))
            self._scrollbar_ver.configure(button_color=kwargs.pop("scrollbar_button_color"))

        if "scrollbar_button_hover_color" in kwargs:
            self._scrollbar_hor.configure(button_hover_color=kwargs.pop("scrollbar_button_hover_color"))
            self._scrollbar_ver.configure(button_hover_color=kwargs.pop("scrollbar_button_hover_color"))
    ...
    def cget(self, attribute_name: str):
        if attribute_name == "width":
            ...
            return self._scrollbar_hor.cget("fg_color")
        elif attribute_name.startswith("scrollbar_button_color"):
            return self._scrollbar_hor.cget("button_color")
        elif attribute_name.startswith("scrollbar_button_hover_color"):
            return self._scrollbar_hor.cget("button_hover_color")
    ...
    def _fit_frame_dimensions_to_canvas(self, event):
        if self._orientation == "horizontal":
            self._parent_canvas.itemconfigure(self._create_window_id_hor, height=self._parent_canvas.winfo_height())
            self._parent_canvas.itemconfigure(self._create_window_id_ver, width=self._parent_canvas.winfo_width())
